...
export const downloadExcel = async (plantId, Authtoken, pageNumber, apiCallInterceptor) => {
    try {
        const encryptedData = await encryptAES(JSON.stringify(plantId))
        const response = await apiCallInterceptor.get(`reports/?page=${pageNumber}&plant_id=${encryptedData}`);
        return response.data;
    } catch (error) {
        return error;
    }
