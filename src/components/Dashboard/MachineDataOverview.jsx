...
// export default MachineDataOverview;
import React, { useState, useEffect } from "react";
import {
  VideoCameraOutlined,
  BugOutlined,
...
import {
  ...
  fetchProducts,
  fetchDashboardData,
  fetchDefectVsMachineData,
} from "../../API/apiCalls";
import { localPlantData, AuthToken, baseURL } from "../../API/API";
...
const MachineDataOverview = () => {
  ...
  const handleApplyFilters = async () => {
    ...
    try {
      const data = await fetchDashboardData(filteredQueryParams);
      setLoaderData(false);
      const { active_products, ...filterData } = data;
      setTableData(filterData);
      setActiveProd(active_products);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const machines = await fetchMachines(localPlantData.plant_name);
        setMachineOptions(machines);
        const products = await fetchProducts(localPlantData.plant_name);
        setProductOptions(products);
        setAlertData(products);
        ...
        <div className="inner-w">
          ...
          <div className="grid grid-cols-4 gap-4 h-[150px]">
            <div className="p-4 bg-white rounded-xl text-left">
              ...
              <div className="text-[40px] text-gray-600 font-bold">{activeProd.length}</div>
            </div>
            <div className="p-4 bg-white rounded-xl text-left">
              <div className="flex justify-between items-center">
                <span>Defect Classification</span>
              ...
              <div className="text-[40px] text-gray-600 font-bold">{tableData.defectClassification || 0}</div>
            </div>
            <div className="p-4 bg-white rounded-xl text-left">
              <div className="flex justify-between items-center">
                <span>No. of SKU</span>
              ...
              <div className="text-[40px] text-gray-600 font-bold">{tableData.skuCount || 0}</div>
            </div>
            <Link className="p-4 bg-white rounded-xl text-left group hover:text-white hover:!bg-red-500">
              <div className="flex justify-between items-center">
                <span>Insights</span>
              ...
              <div className="text-[40px] text-gray-600 font-bold">10</div>
            </Link>
          </div>
        </div>
      </div>
