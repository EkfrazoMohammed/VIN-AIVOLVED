...
const AiSmartView = () => {
  ...
  if (currentSlideIndex === pagination?.pageSize - 1 || 9) {
    ...
    // // Fetch new AI Smart View data
    // const aiSmartViewData = await getAiSmartView(
    //   localPlantData.id, 
    //   AuthToken, 
    //   apiCallInterceptor, 
    //   pagination.currentPage + 1,  // Increment page number here
    //   selectedDefect.id
    // );
    
    // // Reset carousel to first slide and set the new data
    // dispatch(setAismartviewData({ aismartviewData: aiSmartViewData }));
    dispatch(setCurrentSlideIndex(0)); // Reset to the first slide

  } else if (sliderRef.current) {
    // Move to the next slide within the current page of data
